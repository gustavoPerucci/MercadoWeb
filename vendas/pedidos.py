            ...
            if estoque_total >= quantidade_baixa:
                ...
                if saida_automatica > 0 and entrada_automatica > 0:
                    ...
                    if saida.is_valid():
                        ...
                        entre em contato com o suporte técnico através do e-mail: gustavo_perucci@hotmail.com ...'''
                ...
                if saida_automatica > 0 and entrada_automatica > 0 and saida.is_valid() and form_saida.is_valid():
                    ...
                    Por favor contate o suporte técnico através do <br> e-mail: gustavo_perucci@hotmail.com '''
                    sucesso['titulo'] = 'ERRO INTERNO NO SISTEMA...'
                    retorno = json.dumps(sucesso)
                    return HttpResponse(retorno, content_type="application/json")
...
def imprimir_pedido_pdf(request, id_venda):
    ...
    response['Content-Disposition'] = 'inline; filename="MercadoWebPedido_000{}.pdf"'.format(pedido.pk)
    buffer = BytesIO()
    ...
    canv.setFont("Helvetica", 8)
    canv.setAuthor('GustavoPerucci-Sistemas')
    ...
    canv.drawCentredString(285, 25, '*** Mercado Web, © Gustavo Perucci-Sistemas  ({}) ***  Franca - SP - Brasil E-mail: gustavo_perucci@hotmail.com '.format(datetime.today().strftime('%Y')))
    canv.showPage()
    canv.save()
    pdf = buffer.getvalue()
    buffer.close()
